...
import (
	...
	client_daraja "github.com/SirWaithaka/payments-api/clients/daraja"
...
func TestDarajaApi_C2B(t *testing.T) {
	...
	t.Run("test that request is sent and saved", func(t *testing.T) {
		...
		mux.HandleFunc(client_daraja.EndpointC2bExpress, func(w http.ResponseWriter, r *http.Request) {
			// parse request body
			var req client_daraja.RequestC2BExpress
			...
			assert.Equal(t, client_daraja.TypeCustomerPayBillOnline, req.TransactionType)
		...
		// build daraja client
		client := client_daraja.New(client_daraja.Config{Endpoint: server.URL})
		// create instance of daraja service
		service := daraja.NewDarajaApi(&client, client_daraja.SandboxCertificate, shortcode, repository)
...
func TestDarajaApi_B2C(t *testing.T) {
	...
	t.Run("test that request is sent and saved", func(t *testing.T) {
		...
		// create a mock test server
		mux := http.NewServeMux()
		mux.HandleFunc(client_daraja.EndpointB2cPayment, func(w http.ResponseWriter, r *http.Request) {
			// parse request body
			var req client_daraja.RequestB2C
			if err := jsoniter.NewDecoder(r.Body).Decode(&req); err != nil {
				t.Errorf("expected nil error, got %v", err)
			}
			// assert request values
			...
			assert.Equal(t, client_daraja.CommandBusinessPayment, req.CommandID)
			assert.Equal(t, testPayment.Amount, req.Amount)
...
func TestDarajaApi_B2B(t *testing.T) {
	...
	t.Run("test that paybill payment request is sent and saved", func(t *testing.T) {
		...
		// create a mock test server
		mux := http.NewServeMux()
		mux.HandleFunc(client_daraja.EndpointB2bPayment, func(w http.ResponseWriter, r *http.Request) {
			// parse request body
			var req client_daraja.RequestB2B
			if err := jsoniter.NewDecoder(r.Body).Decode(&req); err != nil {
				t.Errorf("expected nil error, got %v", err)
			}
			// assert request values
			assert.Equal(t, shortcode.InitiatorName, req.Initiator)
			assert.Equal(t, client_daraja.CommandBusinessPayBill, req.CommandID)
			assert.Equal(t, testPayment.Amount, req.Amount)
			assert.Equal(t, shortcode.ShortCode, req.PartyA)
			assert.Equal(t, testPayment.ExternalAccountNumber, req.PartyB)
	...
	t.Run("test that till payment request is sent and saved", func(t *testing.T) {
		...
		mux.HandleFunc(client_daraja.EndpointB2bPayment, func(w http.ResponseWriter, r *http.Request) {
			...
			// assert request values
			assert.Equal(t, shortcode.InitiatorName, req.Initiator)
			assert.Equal(t, client_daraja.CommandBusinessBuyGoods, req.CommandID)
			assert.Equal(t, testPayment.Amount, req.Amount)
			assert.Equal(t, shortcode.ShortCode, req.PartyA)
			assert.Equal(t, testPayment.ExternalAccountNumber, req.PartyB)
			assert.Equal(t, testPayment.Beneficiary, req.AccountReference)
...
func TestDarajaApi_Status(t *testing.T) {
	...
	// create a mock test server
	mux := http.NewServeMux()
	mux.HandleFunc(client_daraja.EndpointTransactionStatus, func(w http.ResponseWriter, r *http.Request) {
		// mock request failure
		w.WriteHeader(http.StatusOK)
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(fmt.Sprintf(`{"ResponseDescription":"Success","ResponseCode":"0","ConversationID":"%s","OriginatorConversationID":"%s"}`, conversationID, originatorConversationID)))
