...
import (
	...
	client_quikk "github.com/SirWaithaka/payments-api/clients/quikk"
...
func TestQuikkApi_C2B(t *testing.T) {
	...
	t.Run("test that request is sent and saved", func(t *testing.T) {
		...
		mux.HandleFunc(client_quikk.EndpointCharge, func(w http.ResponseWriter, r *http.Request) {
			// parse request body
			var req client_quikk.RequestDefault[client_quikk.RequestCharge]
		...
		client := client_quikk.New(client_quikk.Config{Endpoint: server.URL})
...
func TestQuikkApi_B2C(t *testing.T) {
	...
	t.Run("test that request is sent and saved", func(t *testing.T) {
		...
		// create a mock test server
		mux := http.NewServeMux()
		mux.HandleFunc(client_quikk.EndpointPayout, func(w http.ResponseWriter, r *http.Request) {
			// parse request body
			var req client_quikk.RequestDefault[client_quikk.RequestPayout]
			if err := jsoniter.NewDecoder(r.Body).Decode(&req); err != nil {
				t.Errorf("expected nil error, got %v", err)
			}
...
func TestQuikkApi_B2B(t *testing.T) {
	...
	t.Run("test b2b to paybill", func(t *testing.T) {
		...
		// create a mock test server
		mux := http.NewServeMux()
		mux.HandleFunc(client_quikk.EndpointTransfer, func(w http.ResponseWriter, r *http.Request) {
			// parse request body
			var req client_quikk.RequestDefault[client_quikk.RequestTransfer]
			if err := jsoniter.NewDecoder(r.Body).Decode(&req); err != nil {
				t.Errorf("expected nil error, got %v", err)
			}
...
func TestQuikkApi_Status(t *testing.T) {
	...
	t.Run("test that if payment has reference txn_id type is used", func(t *testing.T) {
		...
		// create a mock test server
		mux := http.NewServeMux()
		mux.HandleFunc(client_quikk.EndpointTransactionSearch, func(w http.ResponseWriter, r *http.Request) {
			// parse request body
			var req client_quikk.RequestDefault[client_quikk.RequestTransactionStatus]
			if err := jsoniter.NewDecoder(r.Body).Decode(&req); err != nil {
				t.Errorf("expected nil error, got %v", err)
			}
