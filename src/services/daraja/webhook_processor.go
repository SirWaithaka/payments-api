...
type WebhookResult struct {
	...
	ResultCode     daraja2.ResultCode `json:"resultCode"`
...
func c2bWebHookResult(body io.Reader) (WebhookResult, error) {
	...
	var c2bResult daraja2.WebhookRequestC2BExpress
	...
	if c2bResult.Body.StkCallback.ResultCode != daraja2.ResultCodeSuccess {
...
func b2cWebhookResult(body io.Reader) (WebhookResult, error) {
	...
	var b2cResult daraja2.WebhookRequestB2C
	...
	// check if result code is success
	if b2cResult.Result.ResultCode != daraja2.ResultCodeSuccess {
		wb.Status = StatusFailed
		return wb, nil
	}
...
func b2bWebhookResult(body io.Reader) (WebhookResult, error) {
	...
	var b2bResult daraja2.WebhookRequestB2B
	...
	// check if result code is success
	if b2bResult.Result.ResultCode != daraja2.ResultCodeSuccess {
		wb.Status = StatusFailed
		return wb, nil
	}
...
func transactionStatusWebhookResult(body io.Reader) (WebhookResult, error) {
	...
	var searchResult daraja2.WebhookRequestTransactionStatus
	...
	// request failed.
	if searchResult.Result.ResultCode != daraja2.ResultCodeSuccess {
		return wb, nil
	}
...
func (processor WebhookProcessor) Process(ctx context.Context, result *requests.WebhookResult, out any) error {
	...
	case string(daraja2.OperationC2BExpress):
		wb, err = c2bWebHookResult(r)
	case string(daraja2.OperationB2C):
		wb, err = b2cWebhookResult(r)
	case string(daraja2.OperationB2B):
		wb, err = b2bWebhookResult(r)
	case string(daraja2.OperationTransactionStatus):
		wb, err = transactionStatusWebhookResult(r)
	case string(daraja2.OperationReversal):
